...
class ReportsController extends Controller
{
    ...
    public function year() {
        ...
        $f_invoice_no = isset($request->invoice_no) ? $request->invoice_no : '';
        $f_room = isset($request->room) ? $request->room : '';
        $f_date = isset($request->date) ? $request->date : date("Y");
        
        $invoices = $this->getInvoicesByYear($f_branch, $f_room, $f_date, $f_invoice_no, 30);
        $branches = Branch::getBranchByAuth();
        $rooms = Room::all();
    ...
    private function getInvoicesByYear($branch, $room, $date, $invoice_id, $limit=30) {
        ...
        if($date != null) {
            $whereData[] = [ DB::raw("DATE_FORMAT(i.created_at, '%Y')"), $date ];
        }
